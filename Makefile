...
PKG := "github.com/gidyon/rupacinema/account"
...
docker_build: ## Create a docker image for the service
	docker build --no-cache -t rupacinema-account:dev .

docker_run: # Run the rupacinema-account:dev image
	docker run --rm -it -p 11093:11093 -e REDIS_URL=redis rupacinema-account:dev -uflag

docker_rmi: ## Remove docker image called rupacinema-account:dev
	docker rmi rupacinema-account:dev
	
run_app: build_server_prod docker_rmi docker_build docker_run

run:
	go run cmd/server/main.go

prepare_app: build_server_prod
